...
#           You may not be generating in the right ratio without doing this.
...
def createProcessList(readingGroupsList, readDurationSeconds, responseMetricsQueue):
    ...
    for readingGroup in readingGroupsList:
        ...
        for p in range(readingGroup["procs"]):
            process = Process(target=findRandDocsForDuration,args=(readingGroup, readDurationSeconds, responseMetricsQueue))
...
def findRandDocsForDuration(readingGroup, durationSeconds, responseMetricsQueue):
    ...
    print("Starting read workload in " + currentProcName + " (PID: " + str(currentProcId) + "). Reading docs: " + str(docsLower) + "-" + str(docsUpper))    
